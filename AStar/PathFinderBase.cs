...
namespace AStar
{
    public class PathFinderBase<T> : IFindAPath where T : IEquatable<T>
    {
        private readonly T ClosedValue = default(T);
        private const int DistanceBetweenNodes = 1;
        private readonly PathFinderOptions _options;
        private readonly IModelAGrid<T> _world;
        private readonly ICalculateHeuristic _heuristic;

        public PathFinderBase(IModelAGrid<T> worldGrid, PathFinderOptions pathFinderOptions = null)
        {
        ...
        public Position[] FindPath(Position start, Position end)
        {
            ...
            while (graph.HasOpenNodes)
            {
                ...
                {
                    if (_world[successor.Position].Equals(ClosedValue))
                    {
                        continue;
                    }
